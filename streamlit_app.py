...
def _haversine_vec(lat1, lon1, lat2_arr, lon2_arr):
    """Vectorized distance calculation in km."""
    R = 6371.0
...
def load_all_data():
    """Loads all CSV representations of the uploaded Master files."""
    d = {}
    ...
    # 1. Master Clinic Lat/Lon (The Anchors)
    d['clinics'] = _safe_load("Clinics Lat log.xlsx - Clinics Lat log.csv")
    
    # 2. Competitor IVF Database (The Whitespace threat)
    d['ivf_comp'] = _safe_load("IVF_Database_GeoTier_Final.xlsx - Sheet1.csv")
    
    # 3. First Time Customers (True Demand Signals)
    d['demand_clinic'] = _safe_load("First Time customer - Clinic ( 2023 to 2025).xlsx - Data.csv")
    d['demand_web'] = _safe_load("First Time customer - website  (2020 - 2025).xlsx - Sheet1.csv")
    
    # 4. NTB Show Rate (Conversion Efficiency)
    d['show_rate'] = _safe_load("NTB Show% Month Wise.xlsx - Sheet1.csv")
    
    # Helper: pin geocodes for Mapbox
    d['pin_geo'] = _safe_load("pin_geocode.csv")
...
with tab2:
    ...
    if not data['clinics'].empty:
        ...
        st.dataframe(city_clinics[['Clinic Code', 'Clinic Address', 'Latitude', 'Longitude']], use_container_width=True, hide_index=True)
    else:
        st.warning("Clinic Lat/Lon data not found.")
    ...
    if has_map_data:
        ...
        with col_m:
            ...
            fig_map.add_trace(go.Scattermapbox(
                ...
                name='Active Clinics', text=clinics['City']
            ))
        ...
        with col_t:
            ...
            st.caption("Top Cities by IVF Competitor Count (Higher = Tougher Whitespace)")
            if not data['ivf_comp'].empty:
        ...
        st.warning("Awaiting `pin_geocode.csv` or Web Demand data to generate spatial heatmaps.")
...
with tab4:
    ...
    with st.expander("⚙️ Scenario Adjustments (Sensitivity Controls)", expanded=True):
        ...
        conv_rate = sc2.slider("1Cx Conversion %", 20, 100, 100, help="Percentage of First Time Consults that convert to paying patients.")
        capex = sc3.number_input("Capex per Clinic (₹L)", 10.0, 50.0, 28.0)
        opex = sc4.number_input("OpEx per Clinic/Mo (₹L)", 1.0, 10.0, 3.0)

    # Simplified Model Engine
    clinics_to_launch = st.slider("Number of Clinics to Launch", 1, 50, 10)
    
    # Calculate Mature Revenue per Clinic (Assuming roughly 150 1Cx walk-ins per month at mature stage)
    monthly_1cx_walkins = 150 
    mature_rev_lacs = (avg_bill * monthly_1cx_walkins * (conv_rate/100)) / 100
    
    # Ramp logic (Takes 6 months to hit mature revenue)
    ...
    fig_payback.update_layout(
        ...
        xaxis_title="Month post-launch",
        yaxis_title="Cumulative Cashflow (₹L)",
...
with tab5:
    ...
    # ── 1. The O2O Hypothesis (Historical Proof) ──
    st.markdown("#### 1. The O2O Proof Model")
    st.info("💡 **Hypothesis:** When an offline clinic is placed inside a digital hotspot, local trust skyrockets. Analysis of mature clinics indicates that **Offline Monthly Sales ≈ 10X the historical Web Monthly Revenue** from that specific catchment.")
    
    # Mock/Calculated Example of the 10X Proof
    ...
    # Check if web data exists to simulate
    if not data['demand_web'].empty:
        # Aggregate web demand by SubCity/City for the dropdown
        web_sandbox = data['demand_web'].copy()
        web_sandbox['Total'] = pd.to_numeric(web_sandbox['Total'], errors='coerce').fillna(0)
        
        # Calculate monthly average web revenue per cluster (assuming 60 months of data from 2020 to 2025)
        ...
        # Filter out clusters with zero or tiny revenue to keep dropdown clean
        ...
        if not cluster_agg.empty:
            ...
            with sc1:
                # Dropdown for selecting a hotspot
                ...
                o2o_multiplier = st.slider(
                    ...
                    help="How many times larger will offline sales be compared to the pre-launch online revenue?"
                )
            ...
            with sc2:
                # Waterfall Chart to visualize the conversion
                fig_o2o = go.Figure(go.Waterfall(
                    name="O2O", orientation="v",
                    measure=["absolute", "relative", "total"],
                    x=["Current Web Rev/Mo", f"O2O Uplift ({o2o_multiplier}X)", "Projected Clinic Rev/Mo"],
                    textposition="outside",
                    text=[
                        fmt_inr(base_web_rev), 
                        f"+{fmt_inr(projected_offline_sales - base_web_rev)}", 
                        fmt_inr(projected_offline_sales)
                    ],
        ...
        st.warning("Please ensure 'First Time customer - website' data is loaded to use the O2O Simulator.")

# ── FOOTER ─────────────────────────────────────────────────────────────
st.sidebar.markdown("---")
st.sidebar.caption("v2.0-Advanced Edition | System Active")
