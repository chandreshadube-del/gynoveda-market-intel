...
# ═══════════════════════════════════════════════════════════════════════════
# TAB 4: PHASED ROI FORECASTER
# ═══════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown("### Phase-Wise ROI Forecaster")
    st.caption("Model your cashflow dynamically based on staggered, month-by-month clinic launches.")
    ...
    st.markdown("#### 📅 24-Month Launch Schedule & Cashflow")
    ...
    with c_metrics:
        ...
        rm2.metric("Projected Rev (24M)", fmt_inr(total_rev_overall * 1e5))
        rm3.metric("Max Capital Burn", fmt_inr(abs(max_burn) * 1e5), "Peak Investment Required", delta_color="inverse")
        rm4.metric("Net EBITDA (24M)", fmt_inr(total_ebitda_overall * 1e5), delta="Profitable" if total_ebitda_overall > 0 else "Burn Phase")
        
        fig_phased = go.Figure()
        fig_phased.add_trace(go.Bar(
            x=months_labels, y=total_rev_arr, 
            name="Revenue", marker_color='#3b82f6', opacity=0.8
        ))
        fig_phased.add_trace(go.Bar(
            x=months_labels, y=-total_capex_arr, 
            name="CapEx (-)", marker_color='#ef4444', opacity=0.8
        ))
        fig_phased.add_trace(go.Scatter(
            x=months_labels, y=total_opex_arr, 
            name="OpEx", line=dict(color='#f97316', width=2, dash='dot')
        ))
        fig_phased.add_trace(go.Scatter(
            x=months_labels, y=cum_cashflow_arr, 
            name="Cum. Cashflow (J-Curve)", line=dict(color='#10b981', width=3)
        ))
        fig_phased.add_hline(y=0, line_color='black', line_width=1)
        fig_phased.update_layout(
            title=f"Staggered Cashflow & ROI (Total {int(np.sum(launches))} Clinics)",
...
# ── FOOTER ─────────────────────────────────────────────────────────────
st.sidebar.markdown("---")
st.sidebar.caption("v3.0-Advanced Edition | System Active")
